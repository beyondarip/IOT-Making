...
class WaterButton(QPushButton):
    def __init__(self, icon_path, size_text, parent=None):
        ...
        # Wadah ikon dengan ukuran tetap yang lebih besar
        ...
        icon_label.setPixmap(icon_pixmap.scaled(130, 130, Qt.KeepAspectRatio, Qt.SmoothTransformation))
...
class MachineWidget(QWidget):
    def __init__(self):
        ...
        # Wadah video animasi dengan label video
        self.animation_container = QWidget()
        self.animation_container.setFixedHeight(100)
    ...
    def setup_animation(self):
        try:
            self.animation_cap = cv2.VideoCapture('yqq.mp4')  # Ganti dengan nama file video Anda
            if not self.animation_cap.isOpened():
                raise Exception("Tidak dapat membuka video animasi")
    ...
    def update_animation(self):
        try:
            ...
            if ret:
                if frame is None:
                    self.animation_cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                    ret, frame = self.animation_cap.read()
                ...
                scaled_pixmap = QPixmap.fromImage(qt_image).scaled(
                    self.animation_container.width(),
                    self.animation_container.height(),
                    Qt.KeepAspectRatio,
            ...
            print(f"Error pembaruan frame animasi: {str(e)}")
    
    def closeEvent(self, event):
        if hasattr(self, 'animation_cap'):
            self.animation_cap.release()
