...
class ModernCard(QFrame):
    clicked = pyqtSignal(str)  # Changed to emit the title
    ...
    def updateStyle(self):
        ...
        self.setStyleSheet(f"""
            ModernCard {{
                {selected_style if self.is_selected else normal_style}
            }}
            ModernCard:hover {{
                {hover_style}
            }}
        ...
        self.title_label.setStyleSheet(f"""
            QLabel {{
                font-size: 18px;
                font-weight: bold;
                color: {'#3498db' if self.is_selected else '#2c3e50'};
                margin-top: 10px;
                background: transparent;
            }}
        """)
...
class VendingMachine(QMainWindow):
    ...
    def initUI(self):
        ...
        for title, subtitle in sizes:
            ...
            card.clicked.connect(self.selectSize)  # Direct connection, no lambda needed
