...
func (c *Client) RecordQuality(qualityData map[string]float64) error {
	...
	_, err := c.makeRequest("POST", endpoint, qualityData)
	return err
}

// RecordSale mencatat data penjualan
func (c *Client) RecordSale(saleData map[string]interface{}) error {
	endpoint := fmt.Sprintf("machines/%s/record_sale/", c.config.MachineID)
	_, err := c.makeRequest("POST", endpoint, saleData)
	return err
}